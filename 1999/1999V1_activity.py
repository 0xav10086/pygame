...
# 待分析图片读入,预处理
activity = cv.imread('./img/test.png')
gray = cv.cvtColor(activity, cv.COLOR_BGR2GRAY)
...
# 计算并匹配每一组中的每一个数值
    for c in digitCnts:  # c表示每个小轮廓的终点坐标
        ...
        z = z + 1
# 画出来
    cv.rectangle(activity, (gx - 5, gy - 5), (gx + gw + 5, gy + gh + 5), (0, 0, 255), 1)  # 左上角,右下角
# putText参数：图片,添加的文字,左上角坐标,字体,字体大小,颜色,字体粗细
    cv.putText(activity, "".join(groupOutput), (gx, gy - 15), cv.FONT_HERSHEY_SIMPLEX, 0.65, (0, 0, 255), 2)

cv.imshow("Output_image_"+str(i), activity)
cv.waitKey(0)